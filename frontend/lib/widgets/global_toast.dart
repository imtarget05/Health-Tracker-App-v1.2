...
class GlobalToast extends StatefulWidget {
  final Widget child;
  const GlobalToast({Key? key, required this.child}) : super(key: key);
...
class _GlobalToastState extends State<GlobalToast> with TickerProviderStateMixin {
  ...
  // debug log so developers can see events arriving in DevTools/console
  // when testing. Remove or guard with kDebugMode for production if desired.
  // ignore: avoid_print
  print('GlobalToast received event: ${e.type} -> ${e.message}');
        ...
        child: Material(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: Colors.black.withOpacity(0.45), // translucent background
